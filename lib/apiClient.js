...
export const api = {
  ...
  // Baremos
getBaremos: (path) => request(`/api/baremos${path || ''}`),
  crearBaremo:        (b)      => request('/api/baremos', { method: 'POST', body: JSON.stringify(b) }),
  actualizarBaremo:   (id, b)  => request(`/api/baremos/${id}`, { method: 'PUT', body: JSON.stringify(b) }),
  eliminarBaremo:     (id)    => request(`/api/baremos/${id}`, { method: 'DELETE' }),

  // Reset
  resetColeccion: (col) => request(`/api/reset/${col}`, { method: 'DELETE' }),

  // Horarios
  getHorarios:        (params) => request(`/api/horarios${params ? '?' + new URLSearchParams(params) : ''}`),
  crearHorario:       (b)      => request('/api/horarios', { method: 'POST', body: JSON.stringify(b) }),
  actualizarHorario:  (id, b)  => request(`/api/horarios/${id}`, { method: 'PUT', body: JSON.stringify(b) }),
  eliminarHorario:    (id)     => request(`/api/horarios/${id}`, { method: 'DELETE' }),

  // Cédula Hospitalaria
  getPisos:        ()       => request('/api/cedula/pisos'),
  crearPiso:       (b)      => request('/api/cedula/pisos', { method: 'POST', body: JSON.stringify(b) }),
  actualizarPiso:  (id, b)  => request(`/api/cedula/pisos/${id}`, { method: 'PUT', body: JSON.stringify(b) }),
  eliminarPiso:    (id)     => request(`/api/cedula/pisos/${id}`, { method: 'DELETE' }),

  getEspacios:        (params) => request(`/api/cedula/espacios${params ? '?' + new URLSearchParams(params) : ''}`),
  getEspacio:         (id)     => request(`/api/cedula/espacios/${id}`),
  crearEspacio:       (b)      => request('/api/cedula/espacios', { method: 'POST', body: JSON.stringify(b) }),
  actualizarEspacio:  (id, b)  => request(`/api/cedula/espacios/${id}`, { method: 'PUT', body: JSON.stringify(b) }),
  eliminarEspacio:    (id)     => request(`/api/cedula/espacios/${id}`, { method: 'DELETE' }),

  // Configuración del sistema
  getSettings:       ()    => request('/api/settings'),
  updateSettings:    (b)   => request('/api/settings', { method: 'PATCH', body: JSON.stringify(b) }),
  ...
  cambiarPassword:   (id, data) => request(`/api/usuarios/${id}/password`, { method: 'PATCH', body: JSON.stringify(data) }),
};

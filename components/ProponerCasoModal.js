...
import { Send, User, Plus, ChevronDown, Save, X } from 'lucide-react';
import { showToast } from '@/components/ToastMessage';

export default function ProponerCasoModal({ caso, onClose }) {
  const { user } = useAuth();
  const { crearCaso, actualizarCaso, especialistas, crearEspecialista, pacientes } = useData();
  const router = useRouter();

  const esEdicion = Boolean(caso);
  ...
  useEffect(() => {
    if (caso) {
      setForm({
        tipo: caso.tipo || 'electivo',
        paciente: caso.paciente || '',
        equipoQuirurgico: caso.equipoQuirurgico || [],
        asistentesExternos: caso.asistentesExternos || [],
        diagnosticoNombre: caso.diagnosticoNombre || '',
        procedimientoNombre: caso.procedimientoNombre || '',
        duracionEstimadaMin: caso.duracionEstimadaMin || '',
        prioridad: caso.prioridad || 'media',
        observaciones: caso.observaciones || '',
        fechaIngresoEmergencia: caso.fechaIngresoEmergencia || '',
        motivoEmergencia: caso.motivoEmergencia || '',
      });
      const patientObj = caso.pacienteObj || pacientes.find(p => p._id === caso.paciente);
      if (patientObj) {
        setPacienteSeleccionado(patientObj);
      }
    }
  }, [caso, pacientes]);
  ...
  const handleSubmit = e => {
    ...
    if (esEdicion) {
      ...
      crearCaso({
        ...
        showToast('Caso enviado para aprobación.', 'success');
        setSaved(true);
        setTimeout(() => { onClose(); router.push('/mis-casos'); }, 1200);
      }).catch(err => {
        showToast(err.message, 'error');
      });
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[95vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-900">{esEdicion ? 'Editar Caso Quirúrgico' : 'Proponer Caso Quirúrgico'}</h2>
          </div>
        ...
        {saved && (
          <div className="mx-6 mt-4 p-3 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm">
            {esEdicion ? '✓ Caso actualizado exitosamente.' : '✓ Caso enviado para aprobación. Redirigiendo…'}
          </div>
        )}
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-5">
          {esEdicion && (
            <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-xl flex gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-blue-700">Editando caso quirúrgico</p>
                <p className="text-xs text-blue-600 mt-0.5">Los cambios serán guardados y enviados para revisión.</p>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 pb-4">
              <button type="button" onClick={onClose} className="btn-secondary w-full sm:w-auto text-center">Cancelar</button>
              <button type="submit" className="btn-primary w-full sm:w-auto" disabled={saved}>
                <Send size={15} /> {esEdicion ? 'Guardar Cambios' : 'Enviar para Aprobación'}
              </button>
            </div>
          </form>
        </div>
